...
const formSlice = createSlice({
    ...
    reducers: {
        setFormInputData: (state, action) => {
            ...
            const userDetail = {id: nanoid(), details: {name: u.name, email: u.email, address: u.address, contact: u.contact, birthday: u.birthday, password: u.password}}
            // console.log(userDetail)
            state.formInputData.users.push(userDetail)
        },
        removeFormInputData: (state, action) => {
            // console.log(action.payload);
            state.formInputData.users = state.formInputData.users.filter((row) => (
